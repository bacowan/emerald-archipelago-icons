...
def embed_texts(queries, model, device):
...
def retrieve_icons(queries, top_k=5):
    model, preprocess, device = setup()
    query_vectors = embed_texts(queries, model, device)  # shape: (num_queries, 512)
    ...
    for i in range(len(queries)):
        ...
        results.append([(pngs[j], scores[j, i]) for j in top_indices])
    return results
